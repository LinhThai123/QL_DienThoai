...
namespace QuanLyCuaHangBanDienThoai
{
    public partial class QuanLy : Form
    {
        ...
        private void btnLuuDT_Click(object sender, EventArgs e)
        {
            ...
            {
                if (id == "" && name == "" && mtbGia.Text == "" && color == "" && rom == "" && ram == "" && timeBH == "" && cbHang.Text == "" && mtbCanhBao.Text == "")
                    MessageBox.Show("Bạn cần nhập thông tin muốn tìm", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else
                    loadDataToDataGridView(dtgvDienThoai, phoneDao.search(id, name, cbHang.Text, mtbGia.Text, mtbCanhBao.Text, color, rom, ram, timeBH));
            }
            else
            {
        ...
        private void btnLuuKH_Click(object sender, EventArgs e)
        {
            ...
            String gioitinh = mtbGioiTinh.Text; 
            if (btnTimKiemKH.Visible == true)
            {
                if (name == "" && phone == "" && gioitinh == "" )
                    MessageBox.Show("Bạn cần nhập thông tin muốn tìm", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else
                    loadDataToDataGridView(dtgvKH, customerDao.search(name, phone , gioitinh));
            }
            else
            {
                if (name == "" || phone == "" || gioitinh == "")
                ...
                else
                {
                    if (btnThemKH.Visible == true)
                    {
                        ...
                        {
                            if (customerDao.insert(name, phone , gioitinh))
                            {
                    ...
                    {
                        if (customerDao.update(idCustomer, name, phone , gioitinh))
                        {
        ...
        private void dtgvKH_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            mtbGioiTinh.Text = dtgvKH.CurrentRow.Cells[3].Value.ToString(); 
        ...
        {
            //FindByPrice findByPrice = new FindByPrice();
            //    findByPrice.Dock = DockStyle.Fill;
            //    findByPrice.Show();

            //    ReportDocument rp = new ReportDocument();
            //    String path = Path.GetFullPath(@"../../CrytalReport/CrystalReportFindPrice.rpt");
            //    rp.Load(path);
            //    if (mtbGia.Text != "" && mtbGiaMax.Text == "")
            //    {
            //        rp.RecordSelectionFormula = "{showAllPhone.Giá} >=" + mtbGia.Text; 
            //    }
            //    else if(mtbGiaMax.Text != "" && mtbGia.Text == "")
            //    {
            //        rp.RecordSelectionFormula = "{showAllPhone.Giá} <=" + mtbGiaMax.Text;
            //    }
            //    else
            //    {
            //        rp.RecordSelectionFormula = "{showAllPhone.Giá} >=" + mtbGia.Text + " AND {showAllPhone.Giá} <=" + mtbGiaMax.Text;
            //    }
            //    findByPrice.crystalReportFindPrice.ReportSource = rp;
            //    findByPrice.crystalReportFindPrice.Refresh(); 
        }
