...
alter table tblCustomer add gioitinh varchar(3) ; 
...
GO
--View xem danh sách 
Alter VIEW showAllCustomer
AS
	SELECT id AS [ID], name AS [Họ tên], phone AS [SĐT] , tblCustomer.gioitinh AS [GioiTinh]
	FROM dbo.tblCustomer

GO

--Thủ tục thêm tài khoản
Alter PROC insertCustomer (@name NVARCHAR(30), @phone VARCHAR(12) , @gioitinh NVARCHAR(3))
AS
BEGIN
	INSERT INTO dbo.tblCustomer (name, phone , gioitinh)
	VALUES (@name, @phone , @gioitinh)
END
...
GO
--Thủ tục sửa thông tin tài khoản
Alter PROC updateCustomer (@id INT, @name NVARCHAR(30), @phone VARCHAR(12) , @gioitinh NVARCHAR(3))
AS
BEGIN
    UPDATE dbo.tblCustomer
	SET name = @name, phone = @phone , gioitinh = @gioitinh
	WHERE id = @id
END
...
-- View hoa don ban tang dan 
GO
CREATE VIEW vv_HoaDonBan
AS
    SELECT dbo.tblDetailBillOut.billOutId AS [Mã hóa đơn], SUM(tblDetailBillOut.quantity) AS [Số lượng], 
		SUM(tblDetailBillOut.quantity * price) AS [Thành tiền] 
	FROM dbo.tblBillOut 
				JOIN dbo.tblDetailBillOut ON tblDetailBillOut.billOutId = tblBillOut.id 
				JOIN dbo.tblPhone ON tblPhone.id = tblDetailBillOut.phoneId
	GROUP BY billOutId

GO
SELECT * FROM dbo.vv_HoaDonBan ORDER BY [Thành tiền] ASC

-- View Khach hang co sl mua cao nhat 
GO 
Create VIEW vv_KhachHangSlMuaCaoNhat 
AS
    SELECT dbo.tblBillOut.customerId AS [Mã KH] , dbo.tblCustomer.name AS [Tên KH] ,
			Sum(tblDetailBillOut.quantity) AS [Số lượng mua],
			Sum(tblDetailBillOut.quantity * price) As [Thành tiền]
	FROM dbo.tblBillOut 
	             JOIN dbo.tblCustomer on tblBillOut.customerId = tblCustomer.id 
				 JOIN dbo.tblDetailBillOut on tblDetailBillOut.billOutId = tblBillOut.id
				 join tblPhone on tblPhone.id = tblDetailBillOut.phoneId
	group by dbo.tblBillOut.customerId, dbo.tblCustomer.name 

-- View Khách hàng có số lần mua 
Alter VIEW vv_KhachHangSoLanMua 
AS
SELECT tblCustomer.id AS [Mã KH], tblCustomer.name AS [Tên KH], COUNT(tblBillOut.id) AS [Số lần mua]
FROM dbo.tblCustomer 
		JOIN dbo.tblBillOut ON tblBillOut.customerId = tblCustomer.id 
